...
# Where the engine and weights go.
#
# Defaults to `<extension>/runtime`, which is right for a source checkout. A packaged install
# passes a target instead, because the app directory is root-owned and a first-run download must
# not need sudo - the editor calls this with its globalStorage path.
...
for arg in "$@"; do
	case "$arg" in
		--force) FORCE=1 ;;
		--target=*) RUNTIME="${arg#--target=}" ;;
		-h|--help)
			echo "usage: fetch-llm-runtime.sh [--force] [--target=<dir>]"
			exit 0
			;;
		*) echo "fetch-llm-runtime: unknown argument '$arg'" >&2; exit 2 ;;
	esac
done
mkdir -p "$RUNTIME"
